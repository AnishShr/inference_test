...
from inference_test.model.util import load_model
from inference_test.viz.util_camera import to_camera_frame, load_camera_model
...
def parse_arguments():
    ...
    argparser.add_argument(
        ...
        default="src/inference_test/src/inference_test/config/"
    )
...
class Visualizer:
    ...
    def __init__(self, image_topic, forward_model_path, model_type, goal_image_path, context, fps, config_dir_path):
        ...
        # self.config_file = 'src/inference_test/src/inference_test/config/' + self.model_type + '.yaml'
        self.config_file = config_dir_path + self.model_type + '.yaml'

        if os.path.exists(self.config_file):
            with open(self.config_file, 'r') as model_yaml_file:
                self.model_type_params =yaml.load(model_yaml_file, Loader=yaml.SafeLoader) 
        else:
        ...
        self.context_length = self.model_type_params['model_params']['context_size']
    ...
    def image_callback(self, img_msg):
        ...
        if self.context == True:
            ...
            if len(self.deque_images) == self.buffer_length:
                ...
                if self.fps == 15:
                    ...
                    deque_batch_images = deque(maxlen = (self.context_length + 1))

                    for i in range(0, self.buffer_length, self.waypoint_spacing):
                        deque_batch_images.append(self.deque_images[i])
                    
                    self.obs_img = np.concatenate(tuple(deque_batch_images), axis=1)
        ...
        if self.obs_img is not None:
            ...
            # print(f"goal_x, thata: {goal_x, theta}")
            print(f"v, w: {v, w}")
...
if __name__ == "__main__":
    ...
    fps = args.fps
    config_dir_path = args.config_directory_path
